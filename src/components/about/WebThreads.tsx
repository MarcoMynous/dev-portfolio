...
const WebThreads: React.FC<WebThreadsProps> = ({
  ...
  useEffect(() => {
    ...
    if (!ctx) return;
    const { program } = ctx;
    // The OGL uniform map carries different value types (numbers, arrays, booleans).
  ...
  return <div ref={containerRef} className={className} style={{ width: '100%', height: '100%', overflow: 'hidden' }} />;
};
