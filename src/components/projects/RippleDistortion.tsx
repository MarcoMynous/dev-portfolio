...
export type RippleTrigger = 'hover' | 'click' | 'both' | 'none';
export type RippleQuality = 'low' | 'medium' | 'high';

export interface RippleDistortionRef {
  addWave: (x?: number, y?: number, power?: number) => void;
}
...
const RippleDistortion = forwardRef<RippleDistortionRef, RippleDistortionProps>(
  function RippleDistortion(
    ...
    const addWaveFnRef = useRef<((x?: number, y?: number, power?: number) => void) | null>(null);
    ...
    useImperativeHandle(
      ...
      () => ({
        addWave: (x, y, power) => {
          addWaveFnRef.current?.(x, y, power);
        },
      }),
      []
    );
    ...
    useEffect(() => {
      ...
      addWaveFnRef.current = (customX, customY, power = 1) => {
        const targetX = typeof customX === 'number' ? customX : width * 0.5;
        const targetY = typeof customY === 'number' ? customY : height * 0.5;
        setNewWave(targetX, targetY, power);
      };
      ...
      const onMove = (event: PointerEvent) => {
        const cfg = configRef.current;
        if (!cfg.enabled || reduceMotion || cfg.trigger === 'click' || cfg.trigger === 'none')
          return;
      ...
      const onDown = (event: PointerEvent) => {
        const cfg = configRef.current;
        if (!cfg.enabled || reduceMotion || cfg.trigger === 'hover' || cfg.trigger === 'none')
          return;
        const point = localPoint(event.clientX, event.clientY);
        if (!point) return;
