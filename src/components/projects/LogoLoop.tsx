...
export const LogoLoop = React.memo<LogoLoopProps>(function LogoLoop({
  ...
  const effectiveHoverSpeed = useMemo(() => {
    ...
    return 15; // default smooth deceleration on hover
  ...
  const renderLogoItem = useCallback(
    (item: LogoItem, key: React.Key) => {
      if (renderItem) {
        return (
          <li
            className={isVertical ? styles.verticalItem : styles.item}
            key={key}
            role="listitem"
          >
            {renderItem(item, key)}
      ...
      const content = isNodeItem ? (
        <span className={styles.nodeWrapper}>
          {item.node}
          {showTitles && item.title && (
            <span className={styles.logoTitle}>{item.title}</span>
          )}
        </span>
      ) : (
        <img
    ...
    [isVertical, showTitles, renderItem]
  );
  ...
  const logoLists = useMemo(
    () =>
      Array.from({ length: copyCount }, (_, copyIndex) => (
        <ul
          className={isVertical ? styles.verticalList : styles.list}
          key={`copy-${copyIndex}`}
          role="list"
  ...
  return (
    <div
      ref={containerRef}
      className={`${isVertical ? styles.verticalContainer : styles.container} ${className}`.trim()}
    ...
    >
      {fadeOut && (
        <>
          {isVertical ? (
            <>
              <div aria-hidden className={styles.fadeTop} />
              <div aria-hidden className={styles.fadeBottom} />
            </>
          ) : (
            <>
      ...
      <div
        className={isVertical ? styles.verticalTrack : styles.track}
        ref={trackRef}
